 ...
 * @return std::optional<T> The property value if found and correct type, nullopt otherwise
 */
template<typename T>
 ...
 * @param properties The property map
 * @param key The property name to retrieve
 * @param default_value The default value to return if property not found or wrong type
 * @return T The property value if found and correct type, default_value otherwise
 */
template<typename T>
T getPropertyOr(
    const std::map<sdbus::PropertyName, sdbus::Variant>& properties,
...
T getRequiredProperty(
  ...
  auto result = getProperty<T>(properties, key);
  if (!result) {
    throw std::runtime_error("Required property '" + std::string(key) + "' not found or has wrong type");
  }
  return *result;
}
